...
public class ModelHeightWidthValidator implements OptionValidator
{
	...
	public boolean validate(ModelContext modelContext, OptionModel optionModel, String propertyId, Object value) throws OptionValidationException
	{
		...
		if (modelContext.getModelGridContext().getGridLoadedFrom() != null) {
			...
			int setheight  = modelContext.getModelGridContext().getModelGrid().getHeight();
			int setwidth = modelContext.getModelGridContext().getModelGrid().getWidth();
			
			boolean modMade = false;
			
			if (setheight != globalOptionModel.getHeight()) {
				globalOptionModel.setHeight(setheight);
				modMade = true;
			}
			
			if (setwidth != globalOptionModel.getWidth()) {
				globalOptionModel.setWidth(setwidth);
				modMade = true;
			}
