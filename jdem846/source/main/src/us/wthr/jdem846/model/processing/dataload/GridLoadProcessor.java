...
@GridProcessing(id = "us.wthr.jdem846.model.processing.dataload.GridLoadProcessor", name = "Data Grid Load Process", type = GridProcessingTypesEnum.DATA_LOAD, optionModel = GridLoadOptionModel.class, enabled = false)
public class GridLoadProcessor extends GridProcessor
{
	...
	{
		/*
		 * GridLoadOptionModel optionModel = (GridLoadOptionModel)
		 * this.getProcessOptionModel();
		 * 
		 * useScripting = getGlobalOptionModel().getUseScripting();
		 * //tiledPrecaching = JDem846Properties.getProperty(
		 * "us.wthr.jdem846.performance.precacheStrategy"
		 * ).equalsIgnoreCase(DemConstants.PRECACHE_STRATEGY_TILED);
		 * 
		 * tiledPrecaching =
		 * getGlobalOptionModel().getPrecacheStrategy().equalsIgnoreCase
		 * (DemConstants.PRECACHE_STRATEGY_TILED);
		 * 
		 * north = getGlobalOptionModel().getNorthLimit(); south =
		 * getGlobalOptionModel().getSouthLimit(); east =
		 * getGlobalOptionModel().getEastLimit(); west =
		 * getGlobalOptionModel().getWestLimit();
		 * 
		 * getStandardResolutionElevation =
		 * getGlobalOptionModel().getStandardResolutionElevation();
		 * averageOverlappedData =
		 * getGlobalOptionModel().getAverageOverlappedData(); interpolateData =
		 * getGlobalOptionModel().getInterpolateData();
		 * 
		 * //getStandardResolutionElevation =
		 * JDem846Properties.getBooleanProperty
		 * ("us.wthr.jdem846.performance.standardResolutionRetrieval");
		 * //interpolateData = JDem846Properties.getBooleanProperty(
		 * "us.wthr.jdem846.performance.interpolateToHigherResolution");
		 * //averageOverlappedData = JDem846Properties.getBooleanProperty(
		 * "us.wthr.jdem846.performance.averageOverlappedData");
		 * 
		 * latitudeResolution = getModelDimensions().getLatitudeResolution();
		 * //tileHeight =
		 * JDem846Properties.getIntProperty("us.wthr.jdem846.performance.tileSize"
		 * ); tileHeight = getGlobalOptionModel().getTileSize(); cacheHeight =
		 * latitudeResolution * tileHeight; nextCachePoint = north;
		 * 
		 * minimumElevation = Double.NaN; maximumElevation = Double.NaN;
		 */
	}
	...
	public void onLatitudeStart(double latitude) throws RenderEngineException
	{
		 ...
		 * 
		 * double southCache = latitude - cacheHeight - latitudeResolution; try
		 * { unloadDataBuffers(); loadDataBuffers(latitude, southCache, east,
		 * west); } catch (RenderEngineException ex) { throw new
		 * RenderEngineException("Error loading data buffer: " +
		 * ex.getMessage(), ex); }
		 * 
	...
	{
		/*
		 * try { double elev = getElevation(latitude, longitude); if (elev !=
		 * DemConstants.ELEV_NO_DATA) { //ModelPoint modelPoint =
		 * modelGrid.get(latitude, longitude); //if (modelPoint != null) {
		 * //modelPoint.setElevation(elev); modelGrid.setElevation(latitude,
		 * longitude, elev); if (Double.isNaN(minimumElevation)) {
		 * minimumElevation = elev; } else { minimumElevation =
		 * MathExt.min(minimumElevation, elev); }
		 * 
		 * if (Double.isNaN(maximumElevation)) { maximumElevation = elev; } else
		 ...
		 * RenderEngineException("Error processing point elevation: " +
		 * ex.getMessage(), ex); }
		 */
	}
