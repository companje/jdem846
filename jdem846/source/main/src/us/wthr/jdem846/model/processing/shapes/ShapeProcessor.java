...
@GridProcessing(id = "us.wthr.jdem846.model.processing.shapes.ShapeProcessor", name = "Shape Rendering Process", type = GridProcessingTypesEnum.SHAPES, optionModel = ShapeOptionModel.class, enabled = true)
public class ShapeProcessor extends GridProcessor
{
	...
	public void prepare() throws RenderEngineException
	{
		...
		try {
			mapProjection = MapProjectionProviderFactory.getMapProjection("us.wthr.jdem846.render.mapProjection.equirectangularProjection.name", globalOptionModel.getNorthLimit(),
					globalOptionModel.getSouthLimit(), globalOptionModel.getEastLimit(), globalOptionModel.getWestLimit(), globalOptionModel.getWidth(), globalOptionModel.getHeight());
		} catch (MapProjectionException ex) {
			log.warn("Error creating map projection: " + ex.getMessage(), ex);
		}

		canvasProjection = CanvasProjectionFactory.create(CanvasProjectionTypeEnum.getCanvasProjectionEnumFromIdentifier(globalOptionModel.getRenderProjection()), mapProjection, globalOptionModel
				.getNorthLimit(), globalOptionModel.getSouthLimit(), globalOptionModel.getEastLimit(), globalOptionModel.getWestLimit(), modelDimensions.getOutputWidth(), modelDimensions
				.getOutputHeight(), PlanetsRegistry.getPlanet(globalOptionModel.getPlanet()), globalOptionModel.getElevationMultiple(), modelContext.getRasterDataContext().getDataMinimumValue(),
...
//				}
	...
	protected void renderPath(int shapeType, ShapePath path) throws RenderEngineException
	{
		...
		for (LineStroke lineStroke : lineStrokes) {
			...
			// ShapeFill shapeFill = new ShapeFill(color, shapeType, path,
			// lineStroke, fill);
			// try {
			// shapeFill.fill(modelContext.getModelCanvas());
			// } catch (CanvasException ex) {
			// throw new RenderEngineException("Shape fill error: " +
			// shapeType);
			// } catch (ModelContextException ex) {
			// throw new RenderEngineException("Error fetching model canvas: " +
			// ex.getMessage(), ex);
			// }
