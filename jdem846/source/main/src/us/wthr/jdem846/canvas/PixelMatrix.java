...
public class PixelMatrix extends AbstractBuffer
{
	...
	public int[] getRgbaStack(double x, double y)
	{
		...
		if (matrixIndex < 0 || matrixIndex >= zMatrix.length) {
			return null;
		}
	...
	{
		int matrixIndex = this.getIndex(x, y);
		if (matrixIndex < 0 || matrixIndex >= zMatrix.length || zMatrix[matrixIndex] == null) {
			return NO_Z_VALUE;
		} else {
	...
	public void set(double x, double y, double z, int rgba)
	{
		...
		if (isPixelDepthLimited() && stackDepth == 1 && overlayTransparentColors()) {
			...
			if (zMatrix[matrixIndex] == null) {
				zMatrix[matrixIndex] = new float[1];
				rgbaMatrix[matrixIndex] = new int[1];
			} else if (zMatrix[matrixIndex][0] < z) {
			...
			if (stackIndex < 0 || isPixelDepthLimited() && stackIndex >= stackDepth) {
				return;
			}
