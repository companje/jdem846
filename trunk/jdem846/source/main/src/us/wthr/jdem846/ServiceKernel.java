...
public class ServiceKernel extends Thread
{
	...
	protected void initializeService(Class<AppService> clazz) throws ServiceException
	{
		...
		*/
		...
		/*
		Method initialize = getAnnotatedMethod(clazz, Initialize.class);
		if (initialize != null) {
			try {
				initialize.invoke(instance);
			} catch (Exception ex) {
				//ex.printStackTrace();
				log.error("Failed to invoke initialization method on '" + clazzName + "': " + ex.getMessage(), ex);
				throw new ServiceException(clazzName, "Failed to invoke initialization method on '" + clazzName + "'", ex);
			} 
		}
		*/
